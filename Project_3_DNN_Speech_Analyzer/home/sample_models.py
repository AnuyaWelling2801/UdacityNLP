...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(GRU(units, activation="relu", return_sequences=True), name="bi_rnn")(input_data)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    temp_layer = bn_cnn
    for n in range(recur_layers):
        rnn = GRU(units, activation='relu', return_sequences=True, dropout = 0.25,
                       name='rnn_{}'.format(n))(temp_layer)
        bn_rnn = BatchNormalization(name='bn_rnn_{}'.format(n))(rnn)
        temp_layer = bn_rnn
